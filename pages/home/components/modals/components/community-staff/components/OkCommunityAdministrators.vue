<template>
    <div>
        <nuxt-link v-for="user in community.administrators" :to="'/' + user.username" :key="user.id">
            <ok-user-tile :user="user"></ok-user-tile>
        </nuxt-link>
    </div>
</template>
...
<script lang="ts">
    ...
    @Component({
        name: "OkCommunityAdministrators",
        components: {OkUserTile, OkSmartText, OkAppStoreButton, OkPlayStoreButton},
    })
    export default class OkCommunityAdministrators extends Vue {
        @Prop({
            type: Object,
            required: true
