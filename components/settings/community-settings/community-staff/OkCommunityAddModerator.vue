...
<script lang="ts">
    ...
    export default class OkCommunityAddModerator extends Vue {
        ...
        async membersRefresher(): Promise<IUser[]> {
            return this.userService.getCommunityMembers({
                community: this.community,
                exclude: [ CommunityMembersExclusion.moderators ],
                count: OkCommunityAddModerator.infiniteScrollChunkMembersCount
            });
        }
        ...
        openModeratorsSettingsModal() {
            this.modalService.openCommunityModeratorsSettingsModal({
                community: this.community
            });
        }

        onWantsToAddModerator(user: IUser) {
            ...
            return async () => {
                try {
                    ...
                    await this.userService.addCommunityModerator({
                        community,
                        user
                    });
        ...
        onWantsToCancelAddingModerator() {
            const community = this.community;
