...
<script lang="ts">
    ...
    export default class OkCommunityAddBannedUser extends Vue {
        ...
        async membersRefresher(): Promise<IUser[]> {
            return this.userService.getCommunityMembers({
                community: this.community,
                exclude: [],
                count: OkCommunityAddBannedUser.infiniteScrollChunkMembersCount
            });
        }
        ...
        openBansSettingsModal() {
            this.modalService.openCommunityBansSettingsModal({
                community: this.community
            });
        }

        onWantsToBanUser(user: IUser) {
            ...
            return async () => {
                try {
                    ...
                    await this.userService.banCommunityUser({
                        community,
                        user
                    });
        ...
        onWantsToCancelAddingBannedUser() {
            const community = this.community;
